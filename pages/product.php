        ...
        <section class="section-12">
            <div class="row">
                ...
                <div class="swapper-2 col-8 col-sm-12">
                    ...
                    $data = $conn->query($sql);
                    while ($row = $data->fetch()) { ?>
                        <div class="content"><?php echo $row['name_category'] ?></div>
                        <div class="row">
                            <?php
                            include_once("../lib/db.php");
                            $db1 = new database();
                            $conn1 = $db1->connect();
                            $sql1 = "SELECT * FROM products WHERE id_category=" . $row['id'] . " ORDER BY id_prd DESC LIMIT 3";
                            $data1 = $conn1->query($sql1);
                            while ($row1 = $data1->fetch()) { ?>
                                <div class="box-product col-4 col-md-6 col-sm-6">
                                    <div class="product">
                                        <a href="product-detail.php?id=<?php echo $row1['id_prd'] ?>">
                                            <div class="img-product">
                                                <img class="img-prd" src="../asset/img/upload/img_product/<?php echo $row1['img_prd_main'] ?>" alt="">
                                            </div>
                                        </a>
                                        <div class="name-product"><?php echo $row1['name_prd'] ?></div>
                                        <div class="sale-price-product">$<?php echo $row1['cost'] ?></div>
                                        <div class="price-product">$<?php echo $row1['price'] ?></div>
