...
if (count($err) == 0 && isset($_POST['name_prd']) && $numrows == 1) {
    ...
    $db = new database();
    $conn = $db->connect();
    $sql = "update products set name_prd='$name_prd',img_prd_main='$image',img_prd_1='$image',img_prd_2='$image',img_prd_3='$image',id_category='$id_category',detail_1='$detail_1',detail_2='$detail_2',cost='$cost',price='$price' where id_prd=" . $id;
    // use exec() because no results are returned
    $conn->exec($sql);
        ...
        <div class="bg-light rounded h-100 p-4">
            ...
            <form action="" method="post" enctype="multipart/form-data">
                <div class="mb-3">
                    ...
                    <input type="text" name="name_prd" class="form-control" id="" aria-describedby="" value="<?php echo $row1['name_prd']; ?>">
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <select name="id_category" class="form-select mb-3" aria-label="Default select example">
                        ...
                        while ($row = $data->fetch()) { ?>
                            <option value="<?php echo $row['id'] ?>" <?php echo $row['id']==$row1['id_category']?"selected":"" ?>><?php echo $row['name_category']; ?></option>
                        <?php } ?>
                    </select>
                </div>
                <div class="mb-3">
                    <label for="" class="form-label">Mô tả chi tiết 1</label>
                    <textarea name="detail_1" class="form-control" id="" aria-describedby="" cols="30" rows="5" value=""><?php echo $row1['detail_1']; ?></textarea>
                </div>
                <div class="mb-3">
                    <label for="" class="form-label">Mô tả chi tiết 2</label>
                    <textarea name="detail_2" class="form-control" id="" aria-describedby="" cols="30" rows="5" value=""><?php echo $row1['detail_2']; ?></textarea>
                </div>
                <div class="mb-3">
                    <label for="" class="form-label">Giá gốc:</label>
                    <input name="cost" type="" class="form-control" id="" aria-describedby="" value="<?php echo $row1['cost']; ?>">
                </div>
                <div class="mb-3">
                    <label for="" class="form-label">Giá bán:</label>
                    <input name="price" type="" class="form-control" id="" aria-describedby="" value="<?php echo $row1['price']; ?>">
                </div>
                <?php if (count($err) > 0) { ?>
