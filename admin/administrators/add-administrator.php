...
if (count($err) == 0 && isset($_POST['fullname']) && $numrows == 0) {
    ...
    $db = new database();
    $conn = $db->connect();
    $sql = "INSERT INTO administrators (fullname, username, email, phone, mk, id_role, avatar)
            VALUES ('$fullname', '$username', '$email', '$phone', '$password', '$id_role', '$image')";
    // use exec() because no results are returned
    $conn->exec($sql);
...
<div class="container-fluid pt-4 px-4">
    <div class="row vh-30 bg-light rounded align-items-center justify-content-center mx-0">
        <div class="bg-light rounded h-100 p-4">
            <h3 class="mb-4">Thêm Mới Quản Trị Viên</h3>
            <form method="post" enctype="multipart/form-data">
                <div class="mb-3">
                    <label for="" class="form-label">Họ và tên:</label>
                    <input type="" name="fullname" class="form-control" id="" aria-describedby="" placeholder="Nhập họ và tên" value="<?php echo $fullname; ?>">
                </div>
                <div class="mb-3">
                    <label for="" class="form-label">Email:</label>
                    <input type="" name="email" class="form-control" id="" aria-describedby="" placeholder="Nhập Email" value="<?php echo $email; ?>">
                </div>
                <div class="mb-3">
                    <label for="" class="form-label">Sdt:</label>
                    <input type="" name="phone" class="form-control" id="" aria-describedby="" placeholder="Nhập Sdt" value="<?php echo $phone; ?>">
                </div>
                <div class="mb-3">
                    <label for="" class="form-label">Tên đăng nhập:</label>
                    <input type="" name="username" class="form-control" id="" aria-describedby="" placeholder="Nhập username" value="<?php echo $username; ?>">
                </div>
                <div class="mb-3">
                    <label for="" class="form-label">Mật khẩu:</label>
                    <input type="password" name="password" class="form-control" id="" aria-describedby="" placeholder="Nhập mật khẩu" value="<?php echo $password; ?>">
                </div>
                <div class=" mb-3">
                    <label for="" class="form-label">Xác nhận mật khẩu:</label>
                    <input type="password" name="conf-password" class="form-control" id="" aria-describedby="" placeholder="Nhập lại mật khẩu" value="<?php echo $conf_password; ?>">
                </div>
                <div class=" mb-3">
                ...
                <div class="mb-3">
                    ...
                    <select name="id_role" class="form-select mb-3" aria-label="Default select example">
                        ...
                        while($row=$data->fetch()){ ?>
                            <option value="<?php echo $row['id_role'] ?>"><?php echo $row['name_role'] ?></option>
                        <?php } ?>
                    </select>
                </div>
